...
const proseVariants = cva({
  base: [
    ...
    "prose prose-neutral dark:prose-invert prose-a:text-current prose-a:underline prose-a:underline-offset-2 prose-a:hover:text-foreground prose-strong:text-current prose-hr:border-foreground prose-p:first:mt-0 prose-p:last:mb-0 prose-ul:first:mt-0 prose-ul:last:mb-0 prose-li:mt-2 prose-li:first:m-0 prose-img:border prose-img:border-neutral-200 prose-img:rounded-md prose-lead:text-lg/relaxed prose-pre:font-mono prose-pre:rounded-none",
...
export const Prose = ({ className, size, ...props }: ProseProps) => {
  return <div className={cx(proseVariants({ size, className }))} {...props} />
}
